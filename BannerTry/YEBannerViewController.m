...
@interface YEBannerViewController ()<UIScrollViewDelegate>
...
@implementation YEBannerViewController
...
{
    self.bannerSV = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 200)];
    self.bannerSV.contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width * 3, self.bannerFrame.size.height);
...
- (void)addPageControl
{
    self.pc = [[UIPageControl alloc]initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 200) / 2.0, self.bannerFrame.size.height * 0.9, 200, 10)];
    self.pc.numberOfPages = self.images.count;
